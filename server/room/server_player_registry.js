...
xss.extend(
    xss.room.ServerPlayerRegistry.prototype,
    /** @lends {xss.room.ServerPlayerRegistry.prototype} */ {
        /**
        ...
        emitPlayers: function () {
            for (let i = 0, m = this.players.length; i < m; i++) {
                this.players[i].emit(xss.NC_PLAYERS_SERIALIZE, this.serialize(this.players[i]));
            }
        },
